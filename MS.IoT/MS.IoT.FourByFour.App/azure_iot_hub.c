...
//
// Note: this connection string is specific to the device "SimulatorDevice". To configure other devices,
...
//
// To monitor messages sent to device "SimulatorDevice" use iothub-explorer as follows:
//    iothub-explorer HostName=iothubpl66ya4476kha.azure-devices.net;SharedAccessKeyName=service;SharedAccessKey=bqDH8LdWeUtdUcPgcCEq6P07Po7ZYziJ9w0wY79C/oQ= monitor-events "SimulatorDevice"
//
...
bool AzureIoT_ClientConnect(void)
{
	...
	char* connectionString = (char*)malloc(MAX_CONNECTION_STRING_SIZE * 3);;
	...
	Applibs_Result result_config = Config_ReadString(CONFIG_CONNECTION_KEY, connectionString, MAX_CONNECTION_STRING_SIZE);
	...
	iothub_client_handle = IoTHubClient_LL_CreateFromConnectionString(connectionString, MQTT_Protocol);
	...
	else
	{
		...
		if (tree != NULL)
		{
			...
			if (mtreeResult == MULTITREE_OK) {
				mtreeResult = MultiTree_AddLeaf(tree, "SampleProperty2", propertyValue);
			}
		...
		{
			Log_Debug("Failed to allocate multitree\n");
		}
	}
}
	...
	else
	{
		...
		if (str_msg != NULL)
		{
			...
			if (messageReceivedFromIoTHubCb != 0)
				...
				Log_Debug("Warning: no user callback set up for event 'message received from IoT Hub' \n");
	...
	void *userContextCallback)
{
	...
	if (strcmp(methodName, "SampleMethod1") == 0)
	{
		if (directMethodCallFromIoTHubCb)
			directMethodCallFromIoTHubCb(methodName, payload);
		Log_Debug("Method SampleMethod1 called...\n");
	}
	else if (strcmp(methodName, "SampleMethod2") == 0)
	{
		if (directMethodCallFromIoTHubCb)
			directMethodCallFromIoTHubCb(methodName, payload);
		Log_Debug("Method SampleMethod2 called...\n");
	}
	...
	void *userContextCallback)
{
	...
	if (JSON_DECODER_OK == JSONDecoder_JSON_To_MultiTree(temp, &tree))
	{
		...
		if (MULTITREE_OK == MultiTree_GetLeafValue(child, "SampleProperty1", &value))
		{
			const char *valueString = (const char*)value;
			if (twinUpdateFromIoTHubCb)
				twinUpdateFromIoTHubCb(valueString);
			Log_Debug("Property SampleProperty1 changed, new value is %s\n", valueString);
		}
