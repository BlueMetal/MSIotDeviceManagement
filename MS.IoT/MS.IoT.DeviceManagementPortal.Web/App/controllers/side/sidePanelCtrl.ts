module MSIoTDeviceManagementPortal {
	...
	class SidePanelCtrl {
		...
		public reportedFeatures = [];
		public desireFeaturesList = [];
		...
		public init() {
			...
			if (this.deviceTwin.reported.featuresDefinitions !== null) {
                ...
                this.reportedFeatures.forEach((featureDefinition: FeatureDefinition) => {
                    var desiredFeatureStatus = this.deviceTwin.desired.features[featureDefinition.name];
		...
		public updateDesiredFeatures(feature, isActivated) {
		...
		public editCancel(formSidePanel) {
		...
		public noSaveCancel(formSidePanel) {
			this.refresh();
			formSidePanel.$setPristine();
			this.isFormDirty = false;
		}
