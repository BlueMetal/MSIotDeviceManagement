	...
	class GroupsCtrl {
        ...
        public getMergedDevicesIds(): ng.IPromise<string[]> {
            ...
            this.listCustomGroups.forEach((customGroup: CustomGroupItem) => {
                var idx = this.selectedCustomGroups.indexOf(customGroup.id);
                if (idx > -1)
        ...
        public addGroup() {
            if (!this.isAddingGroup) {
                ...
                this.groupsService.createCustomGroup({ where: { operator: LogicalOperators.And, rules: [], groups: [], depth: 0 }, name: "New custom group", count: 0 }).then((customGroupId: string) => {
                    this.groupsService.getCustomGroups().then((listCustomGroups: CustomGroupItem[]) => {
                        this.listCustomGroups = listCustomGroups;
